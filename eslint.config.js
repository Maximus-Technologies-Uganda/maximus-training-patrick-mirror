...
module.exports = [
  ...
  // Relax strict TS rules in api/** for adoption (C5)
  {
    ...
    rules: {
      ...
      // Enforce no-console in API TypeScript (route through structured logger instead)
      'no-console': ['error'],
    },
  },
  ...
  {
    files: ['api/tests/**/*.{ts,tsx}', 'api/**/*.{test,spec}.{ts,tsx}'],
    rules: {
      'no-console': 'off',
    },
  },
  ...
  {
    files: ['frontend/**', 'quote/**', 'todo/**', 'expense/**', 'stopwatch/**', 'repos/**'],
    rules: {
      'no-unused-vars': 'off',
      'no-empty': 'off',
