...
function Initialize-AgentFile($targetFile, $agentName) {
    ...
    if (-not (Test-Path $template)) { $template = Join-Path $repoRoot 'templates/agent-file-template.md' }
    ...
    $planExtract = "- $newLang + $newFramework ($currentBranch)"
    if ($newDb -and $newDb -ne 'N/A') { $planExtract = "$planExtract`n- $newDb ($currentBranch)" }
    if ($newTesting) { $planExtract = "$planExtract`n- Testing: $newTesting ($currentBranch)" }
    $content = $content.Replace('[EXTRACTED FROM ALL PLAN.MD FILES]', $planExtract)
    ...
    $recent = "- ${currentBranch}: Added ${newLang} + ${newFramework}"
    if ($newTesting) { $recent = "$recent; Testing: ${newTesting}" }
...
function Update-AgentFile($targetFile, $agentName) {
    ...
    if ($newTesting -and ($content -notmatch [regex]::Escape($newTesting))) { $content = $content -replace '(## Active Technologies\n)', "`$1- Testing: $newTesting ($currentBranch)`n" }
    if ($content -match '## Recent Changes\n([\s\S]*?)(\n\n|$)') {
        $changesBlock = $matches[1].Trim().Split("`n")
        $changeLine = "- ${currentBranch}: Added ${newLang} + ${newFramework}"
        if ($newTesting) { $changeLine = "$changeLine; Testing: ${newTesting}" }
        $changesBlock = ,$changeLine + $changesBlock
...
if ($newTesting) { Write-Output "- Added testing: $newTesting" }
