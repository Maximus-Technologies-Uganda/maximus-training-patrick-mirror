...
describe('requireJsonContentType middleware', () => {
  function createMocks(method: string, contentType?: string, body: Record<string, unknown> = {}) {
  ...
  it('rejects media types that only start with application/json', () => {
    const { req, res, next, status, json } = createMocks('POST', 'application/jsonp');
  ...
  it('sets Cache-Control: no-store on 415 responses (T087)', () => {
    const { req, res, next, status, json: _json } = createMocks('POST', 'application/jsonp');
