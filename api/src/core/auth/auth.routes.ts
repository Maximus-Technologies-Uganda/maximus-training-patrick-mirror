...
router.post('/login', async (req, res) => {
  const { username, password, idToken } = (req.body ?? {}) as {
    username?: string;
    password?: string;
    idToken?: string;
  };
  ...
  const token = signJwt(
    {
      userId,
      role:
        typeof resolvedRole === 'string' && resolvedRole.trim().length > 0 ? resolvedRole : 'owner',
    ...
    if (raw) {
      ...
      if (h && p && sig) {
        ...
        if (sig === expect) {
          ...
          const _userId = typeof payload.userId === 'string' ? payload.userId : undefined;
          // Avoid console logging per T066
        } else {
          // Avoid console logging per T066
        }
