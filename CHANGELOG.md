...
- Restored full CI coverage for pull requests by removing restrictive path filters from the workflow trigger (`.github/workflows/ci.yml`).

### 2025-11-04 (Local CI helper resilience)

### Changed

- Hardened the local CI helper to run with `set -Eeuo pipefail`, centralize pipeline execution, and surface the original command exit status even when tailing logs (`scripts/test-locally.sh`).

### 2025-11-04 (Quality gate workflow guardrails)

### Fixed

- Replaced invalid job-level `hashFiles` guards in the quality gate workflow with runtime file-detection steps so GitHub Actions can parse the workflow while still skipping Playwright, contract, and Spectral jobs when their inputs are absent (`.github/workflows/quality-gate.yml`).

### 2025-11-03 (Log guard & observability hardening - T073, T075, T078, T079, T081, T083, T088 / DEV-689 → DEV-695)

### Added

- Added regression coverage for redaction, including nested-structure unit assertions and a snapshot spec that verifies no emails, tokens, or cookies leak into structured logs (`api/tests/logging.redaction.spec.ts`, `api/tests/logging.redaction.snapshot.spec.ts`).
- Created contract guardrails that parse the canonical OpenAPI file to ensure error envelope examples expose `details[]` arrays and that `/health` schemas and examples stay in sync (`api/tests/contracts/error.examples.spec.ts`, `api/tests/contracts/health.schema.spec.ts`).
- Introduced a non-gating alerting stub that surfaces sampled 5xx/429 counts in CI summaries while caching the k6 toolchain for faster latency jobs (`scripts/quality-gate/alerting-stub.ts`, `.github/workflows/ci.yml`).

### Changed

- Tightened the API lint guard to forbid raw `console.warn`/`console.error` usage and routed configuration/CORS warnings through structured, redacted outputs with retention metadata (`eslint.config.js`, `api/src/config.ts`, `api/src/middleware/cors.ts`).
- Added a dedicated structured logging helper that enforces retention, redaction, and stderr/stdout routing for application warnings/errors, with coverage confirming audit retention overrides (`api/src/logging/structured.ts`, `api/tests/logging/structured.spec.ts`, `api/src/middleware/logger.ts`).
- Expanded `/health` tests to simulate missing Firebase project identifiers and documented richer rate-limit examples in the OpenAPI contract (`api/tests/health/health.route.spec.ts`, `specs/008-identity-platform/contracts/openapi.yaml`).
- Extended the latency bench to persist sampled HTTP status counts alongside latency summaries so the 5xx/429 alerting stub consumes real data (`scripts/quality-gate/run-latency-bench.ts`).
