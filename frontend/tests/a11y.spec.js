...
test.describe('A11y smoke - primary pages', () => {
  test('index: key controls visible and interactive', async ({ page }) => {
    await page.goto('/');
    await expect(
      page.getByRole('heading', { name: 'Inspirational Quotes' })
    ).toBeVisible();
  ...
  test('todo: aria labels on inputs', async ({ page }) => {
    ...
    await expect(page.locator('#task-title')).toHaveAttribute(
      'aria-label',
      'Task title'
    );
    await expect(page.locator('#task-due')).toHaveAttribute(
      'aria-label',
      'Due date'
    );
  });

  test('expense: controls reachable and contrast check placeholder', async ({
    page,
  }) => {
  ...
  test('stopwatch: controls are labeled and operable', async ({ page }) => {
    ...
    await expect(page.locator('#timer-display')).toHaveAttribute(
      'aria-live',
      'polite'
    );
    ...
    await expect(
      page.getByRole('button', { name: 'Export to CSV' })
    ).toBeVisible();
  });
});
