...
describe('To-Do edge cases (table-driven)', () => {
  ...
  describe('getVisibleTasks with unusual filter inputs', () => {
    ...
    it.each([
      {
        filters: /** @type any */ ({ dueToday: 'nope' }),
        expected: ['A', 'B'],
        label: 'non-boolean dueToday coerces to true (string is truthy)',
      },
      {
        filters: /** @type any */ ({ highPriority: 'yes' }),
        expected: ['B'],
        label: 'non-boolean highPriority coerces to true',
      },
      {
        filters: /** @type any */ ({ dueToday: 0, highPriority: 1 }),
        expected: ['B'],
        label: 'numeric flags: 0 falsy, 1 truthy',
      },
